...
export default function Profile() {
  ...
  const [showSettings, setShowSettings] = useState(false); // ✅ defined
        ...
        <div className="p-6">
          <div className="flex flex-col items-center">
            ...
            <div className="mt-2 flex gap-2 text-xs">
              {['everyone', 'friends', 'selected', 'nobody'].map((opt) => (
                <button
                  key={opt}
                  onClick={() => setStoryPrivacy(opt)}
                  className={`px-2 py-1 rounded-full ${storyPrivacy === opt ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-[var(--text)]'}`}
                >
