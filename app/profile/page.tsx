...
import {
  FaCamera, FaSignOutAlt, FaEdit, FaCheck, FaTimes, FaHeart, FaPlus, FaClock, FaCheckCircle, FaCog, FaArrowLeft
} from 'react-icons/fa';
...
type SettingsKeys = /* ... (same as before) */;
type SettingsType = { /* ... */ };
...
export default function Profile() {
  ...
  const loadUserData = async () => {
    ...
    if (isCrypty) {
      setFollowers(Array(4000000).fill('dummy')); // hack to show 4M
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg)] p-6">
        <div className="w-full max-w-sm bg-[var(--card-bg)] rounded-3xl p-8 border border-[var(--border)]">
          ...
          <input className="w-full bg-gray-700/50 text-[var(--text)] p-3 rounded-xl mb-4" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <input className="w-full bg-gray-700/50 text-[var(--text)] p-3 rounded-xl mb-4" placeholder={isLogin ? '4-digit PIN' : 'Create 4-digit PIN'} type={showPin ? 'text' : 'password'} maxLength={4} value={pin} onChange={(e) => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 4) setPin(v); }} />
          <button onClick={() => setShowPin(!showPin)} className="text-[var(--text)] text-sm">{showPin ? 'Hide PIN' : 'Show PIN'}</button>
          ...
          <p className="text-center text-[var(--text)] cursor-pointer mt-4" onClick={() => { setIsLogin(!isLogin); setError(''); setPin(''); }}>{isLogin ? 'No account? Create one' : 'Already have an account?'}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="p-6">
          <div className="flex flex-col items-center relative">
            <div className="relative">
              ...
              <button onClick={() => fileRef.current?.click()} className="absolute bottom-0 right-0 bg-blue-600 rounded-full p-1.5 border-2 border-[var(--bg)]">
            ...
            <div className="mt-3 text-center">
              {isEditing ? (
                <div className="space-y-2">
                  <input className="bg-gray-800 text-[var(--text)] px-3 py-1 rounded text-center w-full" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Display Name" />
                ...
                <>
                  <h2 className={`text-xl font-bold ${isAdmin ? 'text-yellow-400' : 'text-[var(--text)]'}`}>
                    {displayName}
                    {isVerified && <span className="ml-1 text-blue-500" title="Verified"><FaCheckCircle className="inline" size={18} /></span>}
                    {isAdmin && <span className="ml-2 text-xs bg-yellow-500 text-black px-2 py-0.5 rounded-full">Owner</span>}
                  </h2>
            ...
            <div className="flex gap-6 mt-4 text-center">
              <div>
                <p className="text-[var(--text)] font-bold">{isCrypty ? '4M' : following.length}</p>
                <p className="text-gray-400 text-xs">Following</p>
              </div>
              <div>
                <p className="text-[var(--text)] font-bold">{isCrypty ? '4M' : followers.length}</p>
                <p className="text-gray-400 text-xs">Followers</p>
              </div>
              <div>
                <p className="text-[var(--text)] font-bold">{isCrypty ? '19M' : totalLikes}</p>
                ...
                {userPosts.map((post) => (
                  ...
                  >
                    {post.media?.startsWith('data:image') ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gray-700 text-gray-400 text-xs">🌐 Link</div>
                    )}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black z-50 flex flex-col">
          <div className="flex items-center p-4 bg-gray-900 border-b border-gray-700">
            <button onClick={() => setShowSettings(false)} className="text-white hover:text-gray-300 mr-4"><FaArrowLeft size={24} /></button>
            <h2 className="text-white text-xl font-bold">Settings</h2>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {/* Dark Mode, Language, etc. – same as before */}
            <button onClick={() => setShowSettings(false)} className="w-full bg-red-600 text-white py-2 rounded-xl mt-4 hover:bg-red-700 transition">Close</button>
          </div>
        </div>
      )}
    </>
