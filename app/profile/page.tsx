    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black p-6">
        <div className="w-full max-w-sm bg-gray-800/50 backdrop-blur-md rounded-3xl p-8 border border-gray-700">
          ...
          <input className="w-full bg-gray-700/50 text-white p-3 rounded-xl mb-4" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <input className="w-full bg-gray-700/50 text-white p-3 rounded-xl mb-4" placeholder={isLogin ? "4-digit PIN" : "Create 4-digit PIN"} type={showPin ? 'text' : 'password'} maxLength={4} value={pin} onChange={(e) => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 4) setPin(v); }} />
          <button onClick={() => setShowPin(!showPin)} className="text-gray-400 text-sm">{showPin ? 'Hide PIN' : 'Show PIN'}</button>
          ...
          <div className="relative">
            <div onClick={() => fileRef.current?.click()} className="w-24 h-24 rounded-full bg-gray-700 overflow-hidden cursor-pointer border-4 border-blue-500 hover:opacity-80 transition">
              {photoURL ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-4xl bg-gray-600">{user.email[0].toUpperCase()}</div>
              )}
            </div>
            <input ref={fileRef} type="file" accept="image/*" onChange={handlePhotoUpload} className="hidden" />
            <button onClick={() => fileRef.current?.click()} className="absolute bottom-0 right-0 bg-blue-600 rounded-full p-1.5 border-2 border-black">
