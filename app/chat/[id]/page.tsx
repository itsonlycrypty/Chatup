...
export default function ChatRoom() {
  ...
  const send = async () => {
    ...
    const msg = { id: Date.now().toString(), senderId: user.id, text, timestamp: new Date().toISOString() };
  ...
  return (
    <div className="p-4 h-[85vh] flex flex-col pb-24">
      ...
      <div className="flex gap-2 mt-4">
        <input className="flex-1 bg-gray-800 text-white p-3 rounded-xl" value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Type..." />
        <button onClick={send} className="bg-blue-600 px-6 rounded-xl">Send</button>
      </div>
    </div>
  );
        }
